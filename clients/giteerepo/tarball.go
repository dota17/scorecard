...
var (
	errGitRepoNotFound = errors.New("gitrepo not found")
...
func (handler *tarballHandler) init(ctx context.Context, repo gitee.Project) error {
	...
	if err := handler.getGitRepo(ctx, repo); errors.Is(err, errGitRepoNotFound) {
		log.Printf("unable to get git repo %v. Skipping...", err)
		return nil
	} else if err != nil {
		return sce.WithMessage(sce.ErrScorecardInternal, err.Error())
	}
...
func (handler *tarballHandler) getGitRepo(ctx context.Context, repo gitee.Project) error {
	url := repo.HtmlUrl
	...
	_, err = git.PlainClone(tempDir, false, &git.CloneOptions{URL: url})
	if err != nil {
...
// 	url := repo.HtmlUrl
// 	url = strings.Replace(url, ".git", "/repository/archive/"+repo.DefaultBranch+".zip", 1)
